...
class Simulation:
    ...
    def reset(self):
        ...
        # Response vector: independent random init (same distribution, different draw)
    ...
    def _init_preferences(self, dist):
        ...
        if dist == 1:  # Gaussian
            ...
            # Dim 0: half +1, half -1
            self.prefs[:, 0] = 1.0
            self.prefs[n // 2:, 0] = -1.0
            # Remaining dims: uniform noise in [-eps, eps]
            if k > 1:
                self.prefs[:, 1:] = rng.uniform(-eps, eps, (n, k - 1)).astype(np.float32)
        else:  # 0 = Uniform [-1,1]
            self.prefs[:] = rng.uniform(-1, 1, (n, k)).astype(np.float32)

    def _init_response(self, dist):
        """Initialize response vector as Uniform [-1,1] (always)."""
        n, k, rng = self.n, self.k, self.rng
        self.response = rng.uniform(-1, 1, (n, k)).astype(np.float32)
