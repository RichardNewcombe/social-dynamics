...
The Knowledge Manifold experiment introduces a two-layer environment for the social dynamics simulation. Instead of particles directly sensing a hidden fitness landscape, they interact with a \textbf{knowledge manifold} $M$ — a shared, persistent surface representing the organization's accumulated discoveries. 

Particles (representing teams or individuals) walk on the knowledge manifold. They perform noisy research to "push up" the manifold at their current location. The hidden fitness landscape $F$ acts only as an absolute ceiling; knowledge cannot exceed reality.

This creates a dynamic where teams must collaborate to build knowledge. Because of structural support constraints, a single team cannot build a tall, isolated spire of knowledge. Broad bases of shared understanding are required to support high peaks of specialized expertise.
...
Particle movement in the skill dimensions ($p_0, p_1$) is governed by three forces: knowledge gradient climbing, visionary nudges, and adaptive exploration noise. (Movement in the social dimension $p_2$ is governed by standard social dynamics).
...
\subsection{Reward-Modulated Social Nudge}

Particles maintain an exponential moving average of their reward, $\bar{R}_i$. This reward modulates the social dynamics. After the standard neighbor selection and social averaging, an additional nudge pulls particles toward their most successful neighbors:

\begin{equation}
w_{ij} = 1 + \rho \max(\bar{R}_j, 0)
\end{equation}
\begin{equation}
p_i \leftarrow (1 - \eta_{\text{soc}}) p_i + \eta_{\text{soc}} \sum_{j \in \mathcal{N}(i)} \frac{w_{ij}}{\sum w_{ik}} p_j
\end{equation}

where $\rho$ is the social reward scale. This creates a flow of talent toward the most productive teams.
